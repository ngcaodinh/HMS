...
/**
 * Quản lý thu tạm ứng và hoàn ứng cho hồ sơ nội trú.
 *
 * @param props - Danh sách hồ sơ, summary từ API và callback thao tác do workspace sở hữu.
 * @param props.patients - Snapshot hồ sơ; component lọc nội trú ở phía hiển thị.
 * @param props.summary - Số dư/lịch sử tạm ứng từ backend, null khi chưa tải hoặc tải lỗi.
 * @param props.isLoading - Hiển thị loading cho lịch sử và khóa hoàn ứng khi đang tải.
 * @param props.isSubmitting - Hiển thị loading trên nút thu và khóa thao tác gửi lặp.
 * @param props.onPatientChange - Đồng bộ hồ sơ đang chọn với workspace.
 * @param props.onCreateDeposit - Gửi số tiền nguyên VNĐ, phương thức cash/momo và lý do tùy chọn.
 * @param props.onOpenRefundModal - Mở modal xác nhận hoàn ứng; việc ghi nhận do parent thực hiện.
 * @remarks Số dư và lịch sử chỉ lấy từ server. FE chỉ chuẩn hóa/kiểm tra nhanh số tiền dương; quyền,
 * giới hạn và mutation tài chính vẫn do backend xác thực. UI có trạng thái loading, empty, error
 * cục bộ cho amount và success sau khi parent cập nhật summary.
 */
export function AdvanceManagementScreen({
  ...
  // Lọc hồ sơ nội trú từ snapshot hiện tại để không cho thu tạm ứng trên hồ sơ ngoại trú.
  ...
  // Khi danh sách hoặc mã đang chọn đổi, đồng bộ hồ sơ mặc định với parent; không tạo request nên
  ...
  /**
   * Xác thực nhanh input tiền nguyên VNĐ rồi phát callback tạo giao dịch tạm ứng.
   * Guard số tiền và recordId giữ lỗi ở form; thành công/lỗi request được parent phản hồi qua
   * isSubmitting, summary và toast của workspace.
   */
  ...
  /** Đổi hồ sơ nội trú, cập nhật lựa chọn cục bộ và phát hồ sơ hợp lệ cho workspace. */
...
/**
 * Hiển thị lịch sử tạm ứng theo ba trạng thái loading, empty và dữ liệu thành công.
 * Số tiền được định dạng theo locale Việt Nam và đơn vị VNĐ; component không tự ghi dữ liệu.
 */
