...
/** Định dạng doanh thu theo locale Việt Nam, đơn vị VNĐ; thiếu summary hiển thị dấu gạch ngang. */
function formatVnd(value: number | undefined): string {
  return value === undefined ? '—' : `${value.toLocaleString('vi-VN')} đ`;
}

/** Hiển thị ngày của log theo locale Việt Nam; giá trị lỗi được thay bằng dấu gạch ngang. */
...
/**
 * Hiển thị báo cáo ca và nhật ký giao dịch theo khoảng ngày do workspace tải từ backend.
 *
 * @param props - Summary, log, khoảng ngày và callback tải/xuất báo cáo.
 * @param props.summary - KPI của ca; null khi chưa có dữ liệu hoặc tải lỗi.
 * @param props.logs - Nhật ký giao dịch đã tải, được lọc client theo loại để trình bày.
 * @param props.onExportReport - Xuất snapshot hiện tại; parent chịu trách nhiệm tạo file.
 * @param props.fromDate - Ngày bắt đầu input date, định dạng YYYY-MM-DD.
 * @param props.toDate - Ngày kết thúc input date, định dạng YYYY-MM-DD.
 * @param props.isLoading - Hiển thị loading trong bảng và khóa xuất báo cáo.
 * @param props.errorMessage - Lỗi tải báo cáo được hiển thị trong bảng.
 * @param props.onDateChange - Phát khoảng ngày mới để parent gọi API lại.
 * @remarks Có trạng thái loading, error, empty và success. Các KPI tiền dùng VNĐ nguyên theo summary;
 * component không tự tính doanh thu hay thay thế quyền xem báo cáo của backend.
 */
export function ShiftReportScreen({
  ...
  // Lọc nhật ký chỉ để xem; không thay đổi summary, log gốc hay dữ liệu trên server.
