...
/**
 * Tra cứu snapshot hồ sơ tài chính để chọn đúng bệnh nhân trước khi lập hoặc thu hóa đơn.
 *
 * @param props - Danh sách hồ sơ từ parent, trạng thái tải/lỗi và callback điều phối workflow.
 * @param props.patients - Dữ liệu đã tải từ backend, được lọc theo trạng thái, khoa và từ khóa.
 * @param props.isLoading - Hiển thị loading trong bảng và khóa thao tác tải lại.
 * @param props.errorMessage - Lỗi tải danh sách; UI cho phép gọi lại qua onRefresh.
 * @param props.onSelectPatientForInvoice - Chọn hồ sơ để parent mở lập hóa đơn hoặc bảng kê.
 * @param props.onRefresh - Tải lại snapshot hồ sơ; tùy chọn để màn hình read-only vẫn dùng được.
 * @remarks Có empty state khi bộ lọc không khớp và phân trang 20 dòng/trang. Lọc client không thay
 * thế authorization hay validation của backend; các trường nhận diện chỉ được dùng để tìm/chọn hồ sơ.
 */
export function PatientLookupScreen({
  ...
  // Chỉ lọc snapshot đã nhận từ parent; trạng thái hóa đơn và quyền thao tác vẫn do backend quyết định.
  ...
  // Giới hạn hiển thị 20 hồ sơ mỗi trang để bảng không render toàn bộ snapshot cùng lúc.
  const pageSize = 20;
              ...
              {isLoading ? (
                ...
                <tr>
                  <td className="px-4 py-12 text-center" colSpan={7}>
                    <p className="text-sm font-semibold text-[#ba1a1a]">{errorMessage}</p>
                    <button
                      type="button"
                      onClick={onRefresh}
                      className="mt-3 min-h-[36px] rounded-md border border-[#006096] px-3 py-1.5 text-[12.5px] font-semibold text-[#006096] hover:bg-[#e3f2fd] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0ea5e9] focus-visible:ring-offset-1"
                    >
                      Thử tải lại
                    </button>
                  </td>
