...
/** Giá trị nhập dạng chuỗi để giữ input rỗng/thập phân trước khi parse thành số gửi lên API. */
...
/** Khóa field dùng chung cho lỗi sinh hiệu và lý do khám hiển thị inline. */
type FieldKey = keyof VitalsFormState | 'chiefComplaint';

/**
 * Chuyển detail hồ sơ server thành state form, ưu tiên sinh hiệu mới nhất và dùng assessment cho
 * cân nặng/chiều cao khi chưa có giá trị sinh hiệu tương ứng.
 * @param record Detail hồ sơ đang được bác sĩ mở.
 * @returns State input dạng chuỗi, với chuỗi rỗng cho dữ liệu null/không có.
 */
...
/**
 * Tính BMI từ cân nặng kg và chiều cao cm, làm tròn một chữ số thập phân và phân loại theo ngưỡng.
 * @param weightKg Cân nặng theo kilogram ở dạng chuỗi từ input.
 * @param heightCm Chiều cao theo centimeter ở dạng chuỗi từ input.
 * @returns Giá trị BMI, nhãn phân loại và tone hiển thị; `null` khi thiếu hoặc chưa parse được
 * dữ liệu.
 */
function computeBmi(
  weightKg: string,
...
/**
 * Cấu hình label, đơn vị, bước nhập và khoảng gợi ý cho input của từng sinh hiệu.
 * Validation nghiệp vụ ở helper và backend mới quyết định dữ liệu có được lưu hay không.
 */
...
/**
 * Nhập sinh hiệu và bệnh sử cho hồ sơ đang mở, đồng thời kiểm tra dữ liệu trước khi lưu.
 *
 * Form sở hữu local state cho input, lỗi inline, khóa chỉnh sửa và thông báo feedback; mutation
 * `useRecordVitalSignsAndAssessment` gửi dữ liệu lên server và invalidate hồ sơ khi thành công.
 * Trạng thái loading thể hiện qua nút lưu, lỗi field hiển thị tại từng input, lỗi hệ thống qua
 * modal, còn hồ sơ đã đóng bị khóa và không cho mutation. Component không tự cấp quyền; quyền
 * truy cập và validation cuối cùng vẫn thuộc API/backend.
 *
 * @param doctorName Tên bác sĩ dùng trong thông báo xác nhận sinh hiệu.
 * @param record Detail hồ sơ server gồm sinh hiệu, assessment, version và status hiện tại.
 * @returns Giao diện sinh hiệu và bệnh sử với trạng thái rỗng/lỗi/thành công tương ứng.
 */
export function VitalsScreen({
  ...
  // Khi dependency record đổi do query detail mới, đồng bộ local form và xóa lỗi của record trước;
  // effect không đăng ký external resource nên không có cleanup.
  useEffect(() => {
  ...
  /**
   * Tạo handler cập nhật một input và xóa lỗi liên quan; hai giá trị huyết áp được kiểm tra
   * theo cặp.
   * @param field Khóa sinh hiệu cần cập nhật.
   * @returns Handler nhận input event từ trường tương ứng.
   */
  ...
  /**
   * Kiểm tra field khi rời input, gồm ràng buộc đồng thời giữa huyết áp tâm thu và tâm trương.
   * @param field Khóa field vừa blur.
   * @param value Giá trị hiện tại của field ở dạng chuỗi.
   */
  ...
  /** Kiểm tra lý do khám không được để trống sau khi loại bỏ khoảng trắng. */
  function validateChiefComplaint(value: string): string {
    return value.trim() ? '' : 'Vui lòng nhập lý do khám bệnh.';
  }

  /** Tổng hợp lỗi của toàn bộ sinh hiệu, huyết áp và lý do khám trước mutation. */
  ...
  /**
   * Xử lý nút mở khóa/lưu: validate local, gửi mutation theo version và hiển thị feedback kết quả.
   * Khi hồ sơ đang khóa, lần nhấn đầu chỉ mở form; khi mutation lỗi, lỗi validation từ server được
   * map vào field và lỗi còn lại hiển thị qua modal.
   */
    ...
    <div className="grid gap-4 xl:grid-cols-2">
      ...
      <div className="flex justify-end xl:col-span-2">
        <button
          ...
          onClick={() => {
            // Guard này giữ cho hồ sơ đóng không thể gọi save nếu callback bị kích hoạt
            // ngoài trạng thái disabled.
