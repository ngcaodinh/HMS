...
import { FieldError, getFieldErrorMap } from './FieldError';
import { doctorWorkspaceStyles as styles } from '../pages/workspace/doctor-workspace.styles';

// Số ký tự tối thiểu sau khi trim cho lý do ghi đè dị ứng hoặc kê dài ngày.
...
/**
 * Tiền kiểm tra số lượng hoặc số ngày dùng ở client khi người dùng thay đổi ô số.
 *
 * @param value Giá trị thô trong ô nhập; giữ dạng chuỗi để báo lỗi ngay trên form.
 * @param field Trường số lượng hoặc số ngày dùng cần kiểm tra.
 * @returns Thông báo lỗi tiếng Việt hoặc chuỗi rỗng nếu qua tiền kiểm tra.
 * @remarks Mốc 90 ngày chỉ là giới hạn hiển thị/tiền kiểm tra; backend vẫn là nguồn quyết định.
 */
...
/**
 * Chuẩn hóa lỗi nghiệp vụ kê đơn thành thông báo phù hợp với luồng bác sĩ.
 * Lỗi không phải `ApiError` được giữ thông báo dự phòng để không làm lộ chi tiết không cần thiết.
 */
...
/**
 * Đối chiếu heuristic hoạt chất với thông tin dị ứng để cảnh báo trước khi thêm thuốc vào draft.
 *
 * @param medicine Thuốc đang được chọn từ danh mục.
 * @param patientAllergies Chuỗi dị ứng lấy từ hồ sơ; đây là dữ liệu y tế nhạy cảm.
 * @returns Thành phần dị ứng khớp để hiển thị cảnh báo, hoặc `null` nếu chưa phát hiện khớp.
 * @remarks Cảnh báo này chỉ hỗ trợ UI; backend mới quyết định việc chấp nhận allergy override.
 */
...
/** Tạo một dòng kê đơn cục bộ với giá trị khởi tạo có thể tiếp tục chỉnh sửa trước khi lưu. */
...
/**
 * Hiển thị và điều phối luồng kê đơn ngoại trú của bác sĩ cho một hồ sơ bệnh án.
 *
 * @param record Hồ sơ hiện tại, gồm dị ứng bệnh nhân, phiên bản hồ sơ và trạng thái đóng/mở.
 * @remarks
 * - Dữ liệu đơn gần nhất đến từ `useLatestPrescription`; danh mục thuốc và các mutation đi qua
 *   các hook API của module, sau đó hook tự làm mới cache hồ sơ/đơn.
 * - Chưa có đơn hoặc đơn gần nhất đã `cancelled` là vùng nhập cục bộ; `draft` là đơn đã lưu nhưng
 *   chưa ký; `active` và `xml_exported` được giao diện coi là đã ký theo status từ server.
 * - Cảnh báo dị ứng và giới hạn ngày là tiền kiểm tra của client. Backend mới quyết định allergy
 *   override, chữ ký (`isSigned`/`signedAt`), chuyển trạng thái, snapshot giá và trừ kho FEFO.
 * - Có trạng thái tải đơn, trạng thái rỗng/không dùng thuốc, cảnh báo lỗi qua modal và khóa
 *   thao tác khi hồ sơ `closed`; xuất XML mở liên kết proxy sau khi server hoàn tất export.
 * - Khóa hiển thị này không thay thế authorization phía backend; nội dung hồ sơ và dị ứng là dữ
 *   liệu y tế nhạy cảm, chỉ dùng trong ngữ cảnh hồ sơ bác sĩ hiện tại.
 */
export function PrescriptionScreen({ record }: { record: MedicalRecordDetail }) {
  ...
  /** Reset state nhập cục bộ khi dependency `record.recordId` đổi; không có external subscription
   * hoặc cleanup, còn latest prescription được query hook tải theo record mới. */
  useEffect(() => {
    setLines([]);
    setNoDrug(false);
    setChronic(false);
  ...
  // `cancelled` quay lại vùng nhập; `draft` vẫn là đơn server chưa ký và không được xem là active.
  const isLocalPhase = !latestPrescription || latestPrescription.status === 'cancelled';
  // UI suy ra đã ký từ status server; không tự đặt `isSigned`, `signedAt` hay trạng thái active
  // ở client.
  const isSigned =
  ...
  // `days` đang là chuỗi draft; lấy số ngày lớn nhất để áp dụng ngưỡng 30/90 cho cả đơn.
  ...
  /** Xử lý chọn thuốc: chặn hồ sơ đóng, «Không dùng thuốc» và thuốc trùng; conflict mở modal
   * override. */
  ...
  /** Xác nhận lý do ghi đè dị ứng tối thiểu 15 ký tự và chỉ thêm thuốc vào draft cục bộ. */
  ...
  /** Đổi lựa chọn «Không dùng thuốc» và xóa các dòng/lỗi/override cục bộ khi bật lựa chọn này. */
  ...
  // Client chặn >30 ngày nếu thiếu xác nhận bệnh mãn tính/lý do và luôn chặn quá 90 ngày.
  ...
  /** Cập nhật số lượng/số ngày dạng chuỗi và ghi lỗi tiền kiểm tra cho đúng dòng thuốc. */
  ...
  /** Kiểm tra toàn bộ dòng trước submit để hiển thị lỗi cục bộ trước khi gọi backend. */
  ...
  /**
   * Lưu đơn nháp và tùy chọn gọi tiếp luồng ký trên đơn vừa tạo.
   *
   * @param signAfter `true` khi thao tác xác nhận ký ngay sau khi tạo draft.
   * @remarks Request gửi phiên bản kỳ vọng để backend kiểm tra tính nhất quán; backend quyết định
   *   kết quả lưu, allergy override, chữ ký và các tác động tồn kho/FEFO.
   */
  ...
  /** Xử lý click xác nhận ký: ký draft đã lưu hoặc lưu draft cục bộ rồi ký; lỗi hiện qua modal. */
  ...
  /** Xử lý hủy đơn nháp với guard hồ sơ/đơn; mutation server quyết định kết quả và lỗi được
   * hiển thị. */
  ...
  /** Xử lý submit lý do hủy đơn đã ký; backend quyết định đơn còn được phép hủy theo lifecycle. */
  ...
  /** Xử lý xuất XML: chỉ gọi mutation khi status là `active`, sau đó mở file qua proxy. */
