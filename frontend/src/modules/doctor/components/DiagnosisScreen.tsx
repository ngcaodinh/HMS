...
/** Nhãn hướng điều trị và tab workflow tương ứng sau khi chẩn đoán được lưu. */
...
/**
 * Nhập chẩn đoán chính bằng mã ICD-10 và lưu hướng điều trị cho hồ sơ đang mở.
 *
 * Catalog ICD-10 và prescription hiện tại là server state; nội dung tìm kiếm, mã đã chọn, diễn
 * giải, hướng điều trị chờ xác nhận và lỗi là local state. Mutation gửi chẩn đoán lên server,
 * callback `onDiagnosed` chỉ chạy sau khi lưu thành công; page chỉ chuyển workflow ngoại trú sang
 * Tab 5A. Trạng thái đã chẩn đoán cho phép sửa, hồ sơ đóng bị khóa, loading mutation hiển thị
 * trên nút, lỗi validation hiển thị inline và lỗi API qua modal. UI lock không thay thế
 * authorization/backend validation.
 *
 * @param onDiagnosed Callback tùy chọn nhận hướng điều trị sau mutation thành công.
 * @param record Detail hồ sơ server gồm chẩn đoán, version và status hiện tại.
 * @returns Giao diện chọn ICD-10, nhập diễn giải và chọn hướng điều trị.
 *
 * @remarks ICD-10 được tìm qua `GET /clinical-catalogs/icd-10` theo keyword và ngày hiệu lực;
 * việc lưu chẩn đoán do mutation server quyết định, không phải local state.
 */
export function DiagnosisScreen({
  ...
  // Khi record.diagnosis hoặc record.recordId đổi, đồng bộ bản nháp với chẩn đoán server;
   ...
   *
   * @param nextTreatmentType Hướng điều trị bác sĩ vừa chọn.
   */
  ...
  /**
   * Kiểm tra ICD-10, diễn giải và hướng điều trị rồi gửi chẩn đoán theo version hồ sơ.
   * Sau khi thành công, khóa bản nháp và báo page chuyển bước; lỗi field từ server được map
   * vào inline error, lỗi còn lại hiển thị qua feedback modal.
   */
              ...
              <label className="block">
                ...
                <span className="relative block">
                  ...
                  <input
                    ...
                    onBlur={() => {
                      // Chỉ báo lỗi khi rời ô mà chưa chọn entry hợp lệ từ catalog server.
                      if (!selectedIcd)
                ...
                <div className="absolute left-0 right-0 top-full z-10 mt-1 max-h-60 overflow-auto rounded-[12px] border border-[#bfc7d2] bg-white shadow-lg">
                  {icdResults?.map((entry) => (
                    <button
                      ...
                      key={entry.code}
                      onClick={() => {
                        // Entry được chọn từ kết quả server trở thành ICD-10 chính của bản nháp.
              ...
              <span className="inline-flex items-center gap-2 rounded-md bg-[#006096] px-3 py-2 text-xs font-bold text-white shadow-[0_1px_2px_rgba(0,96,150,0.25)]">
                ...
                {!isLocked && (
                  <button
                    ...
                    onClick={() => {
                      // Xóa lựa chọn để buộc bác sĩ chọn lại ICD-10 trước khi submit.
                      setSelectedIcd(null);
                      setFieldErrors((current) => ({
                        ...current,
                        icd10: 'Vui lòng chọn mã ICD-10.',
          ...
          >
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button
                ...
                onClick={() => {
                  // Chỉ cập nhật local draft sau khi bác sĩ đã xác nhận cảnh báo đổi hướng
                  // điều trị.
...
/** Hiển thị một lựa chọn hướng điều trị và báo trạng thái đang được chọn. */
