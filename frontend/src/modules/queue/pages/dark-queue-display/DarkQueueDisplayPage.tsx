...
// Múi giờ IANA dùng cho ngày/giờ pháp lý trên bảng LED, không phụ thuộc timezone của máy hiển thị.
...
/** Hiển thị số thứ tự với tối thiểu bốn chữ số, không làm thay đổi số backend trả về. */
function formatTicketNumber(ticketNumber: number) {
  return String(ticketNumber).padStart(4, '0');
}

/** Định dạng giờ `HH:mm` theo múi giờ Việt Nam cho đồng hồ và lịch sử gọi số. */
...
/** Định dạng ngày `dd/mm/yyyy` theo múi giờ Việt Nam trên header màn hình. */
...
/** Đổi timestamp ISO của backend sang giờ hiển thị; thiếu timestamp thì dùng `--:--`. */
...
/**
 * Màn LED công khai hiển thị số đang gọi và lịch sử phục vụ, không hiển thị PII.
 *
 * @remarks Không nhận props. Snapshot ban đầu và refresh dùng `GET /api/v1/public/queue-display`;
 * socket public cập nhật event realtime, polling 15 giây làm fallback. Loading hiển thị bằng số
 * placeholder, danh sách rỗng có trạng thái empty, còn API/socket failure chỉ đánh dấu offline
 * và giữ dữ liệu đang có; không có forbidden state riêng vì endpoint public. Component không có
 * mutation/print và không dùng dữ liệu bệnh nhân. Access public/no-JWT của endpoint không thay thế
 * authorization cho các màn hình nhân viên.
 */
export function DarkQueueDisplayPage() {
  ...
  /** Nạp snapshot public, map DTO thành số hiện tại/lịch sử/thống kê và giữ UI cũ khi request lỗi. */
  ...
  // Đồng hồ đồng bộ với browser timer; chỉ dừng interval khi màn hình bị tháo khỏi DOM.
  useEffect(() => {
  ...
  /**
   * Đồng bộ external socket và REST fallback khi mount, rồi cleanup mọi listener/subscription/timer.
   * Các event connect, realtime và polling có thể cùng gọi `loadBoard`; cleanup không hủy request
   * đang chạy mà chỉ ngăn trigger mới sau unmount, nên không phải stale-response guard.
   */
  useEffect(() => {
