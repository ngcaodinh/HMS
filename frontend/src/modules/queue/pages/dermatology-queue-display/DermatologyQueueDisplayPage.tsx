...
/** Cấu hình mục điều hướng; cờ emergency chỉ điều khiển nhãn/cảnh báo hiển thị trong bản dựng. */
type NavItem = {
  description: string;
...
/** Dòng hàng đợi minh họa; `number` là chuỗi đã format và `time` dùng định dạng `HH:mm`. */
type TicketItem = {
  action: string;
  number: string;
  time: string;
...
/** Hợp đồng field hành chính minh họa; `required` điều khiển dấu bắt buộc và `aria-required`. */
type FieldProps = {
  id: string;
  label: string;
  placeholder?: string;
...
// Dữ liệu tĩnh để dựng giao diện, không phải snapshot từ API và không được dùng làm nguồn queue thật.
...
/** Chọn biểu tượng điều hướng theo vùng nghiệp vụ và trạng thái active hiện tại. */
...
/** Render tiêu đề section cùng biểu tượng nghiệp vụ tương ứng. */
...
/** Render field biểu mẫu với nhãn, placeholder, kiểu dữ liệu và cờ bắt buộc đã truyền vào. */
...
/**
 * Bản dựng tĩnh workspace tiếp nhận và hàng đợi của khoa da liễu.
 *
 * @remarks Component không nhận props, không gọi API/socket và không có state nên không có
 * loading/error/success state hay mutation queue; danh sách số và biểu mẫu lấy từ constants/JSX
 * tại chỗ. Các nút nghiệp vụ chưa có callback, còn `LogoutButton` tự quản lý side effect đăng xuất.
 * Đây không phải public LED/no-JWT display: giao diện có khu vực nhân viên và trường hành chính,
 * vì vậy route/middleware/backend phải đặt access boundary phù hợp; UI không thay authorization.
 * Không coi dữ liệu tĩnh trong component là dữ liệu bệnh nhân hoặc nguồn sự thật của hàng đợi.
 */
