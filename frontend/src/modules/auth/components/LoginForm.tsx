...
import { LoginSuccessOverlay } from '@/modules/auth/components/LoginSuccessOverlay';
...
/**
 * Dữ liệu thành công do BFF trả về sau khi backend xác thực và tạo session.
 * `homePath` null nghĩa là chưa có route mặc định; `mustChangePassword` ưu tiên workflow bảo mật.
 */
type LoginResponse = {
...
/**
 * Tham số điều khiển thông báo đầu vào của form đăng nhập.
 * `initialReason` chỉ nhận diện lý do điều hướng như `session_expired`; giá trị khác được bỏ qua.
 */
type LoginFormProps = {
...
/**
 * Lưu hoặc xóa username đã nhớ theo lựa chọn của người dùng.
 *
 * @param username - Username hiển thị lại ở lần mở form sau; không phải dữ liệu xác thực.
 * @param shouldRemember - Khi false, xóa giá trị khỏi localStorage.
 * @remarks Chỉ lưu username; localStorage không được chứa password, token hoặc secret.
 */
...
/**
 * Hiển thị form đăng nhập cho cổng nhân viên và điều phối chuyển hướng sau khi xác thực thành công.
 *
 * @param props - Lý do điều hướng tùy chọn để hiển thị thông báo phiên đã hết hạn.
 * @param props.initialReason - Khi bằng `session_expired`, hiển thị hướng dẫn đăng nhập lại.
 * @returns Form với trạng thái loading, lỗi theo trường, lỗi hệ thống, giới hạn thử lại và thành
 * công; overlay thành công giữ giao diện trong lúc chuyển route.
 * @remarks
 * - Validate tại client chỉ phục vụ UX; request `POST /api/auth/login` qua BFF và backend mới là
 *   nơi xác thực, phân quyền và tạo session.
 * - BFF trả principal/homePath; form lưu username nếu người dùng chọn remember, không lưu password
 *   hoặc token, rồi điều hướng tới `/change-password` hoặc homePath.
 * - Lỗi `ApiError` được ánh xạ an toàn; 429 dùng `retryAfterSeconds` để khóa submit và đếm ngược.
 * - BFF/backend có thể làm session hết hạn; thông báo `session_expired` chỉ là tín hiệu UI và không
 *   thay thế authorization ở server.
 */
export function LoginForm({ initialReason }: LoginFormProps) {
  ...
  // Chạy một lần khi mount để đồng bộ username đã nhớ từ localStorage; không tạo subscription nên
  // không có cleanup và tuyệt đối không đọc password hoặc token.
  useEffect(() => {
  ...
  // Khi API trả 429, đồng bộ countdown với timer ngoài React; dọn interval mỗi lần dependency đổi
  // hoặc component unmount để không tạo nhiều timer cạnh tranh.
  useEffect(() => {
  ...
  // Khi đăng nhập thành công, hẹn router.replace tới đích do BFF trả về; cleanup timeout để tránh
  // điều hướng stale sau khi trạng thái đổi hoặc component unmount.
  useEffect(() => {
  ...
  /**
   * Xử lý sự kiện submit form đăng nhập qua BFF.
   *
   * @param event - Sự kiện submit form đăng nhập của người dùng
   * @returns Promise<void>; cập nhật trạng thái loading/lỗi hoặc trạng thái success để effect điều
   * hướng.
   * @remarks
   * Validate client chỉ là UX trước khi gửi; backend vẫn kiểm tra request và credentials cuối cùng.
   * Thành công sẽ lưu username theo lựa chọn remember và chuyển tới change-password hoặc homePath.
   * Lỗi 429 giữ `retryAfterSeconds` để khóa submit; lỗi khác đi qua mapper an toàn và không ghi
   * password, token hoặc secret.
   */
