...
/** Giữ focus trong dialog để thao tác Tab không đi xuyên qua lớp overlay phía sau. */
...
function UsersContent({ principal }: { principal: ItPrincipal }) {
  ...
  /** Thực hiện khóa/mở khóa sau khi operator đã xác nhận và nhập reason tối thiểu 10 ký tự. */
  ...
  /** Gửi reason do operator nhập để reset mật khẩu và chỉ giữ temporary password trong dialog. */
...
export function ItTechnicianWorkspace({ principal }: ItTechnicianWorkspaceProps) {
  ...
  /** Làm mới các query staff đang hiển thị mà không làm mất trạng thái điều hướng của workspace. */
  const handleRefresh = () => {
    void queryClient.invalidateQueries({ queryKey: ['staff-users'] });
  };
