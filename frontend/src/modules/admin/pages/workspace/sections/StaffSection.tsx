...
/** Định dạng thời điểm đăng nhập ISO theo `vi-VN`; giữ nguyên chuỗi nếu dữ liệu không parse được. */
...
/**
 * Bảng nhân sự và các callback yêu cầu thao tác từ component cha.
 ...
 * @param onToggleStatus - Đổi trạng thái active/locked trên state cục bộ.
 * @remarks Khi danh sách rỗng, bảng hiển thị empty state; quyền thật không được quyết định ở đây.
 */
function StaffTable({
  onDeleteRequest,
  onEditRequest,
...
/**
 * Modal thêm/sửa nhân sự với lỗi inline từ schema.
 * @param mode - `create` hiển thị form mới, `edit` nạp dữ liệu đang sửa.
 * @param formValues - Draft cục bộ trước khi submit.
 * @param errors - Map lỗi theo field; không có loading hoặc gọi API trong modal.
 * @param onChange - Cập nhật draft khi input thay đổi.
...
/**
 * Đọc dữ liệu form đã qua Zod và tạo bản ghi nhân sự mới cho state cục bộ.
 * ID sinh bằng `crypto.randomUUID`, thời điểm tạo dùng ISO 8601 và trạng thái mặc định là `active`.
 */
...
/**
 * Màn hình quản lý nhân sự và trạng thái tài khoản trong workspace Admin.
 * @param staffList - Nguồn dữ liệu danh sách hiện tại từ hook state cục bộ.
 * @param dispatchStaffDirectory - Dispatch ADD/UPDATE/DELETE/TOGGLE_STATUS sau khi guard hợp lệ.
 * @param showToast - Callback thông báo kết quả thao tác do workspace sở hữu.
 * @remarks Form có validation inline và guard trùng username; CRUD chỉ thay đổi mock state, không gọi API.
 * UI hiển thị quyền/vai trò nhưng không thay thế authorization backend.
 */
export function StaffSection({ dispatchStaffDirectory, showToast, staffList }: StaffSectionProps) {
  ...
  // Click "Thêm nhân viên": xóa bản ghi đang sửa, reset draft/lỗi rồi mở modal create.
  ...
  // Click hủy/đóng: chỉ đóng modal, giữ nguyên danh sách nhân sự cục bộ.
  const handleCloseModal = () => setModalMode('closed');

  // Input change: cập nhật draft; schema và guard trùng username chỉ chạy khi submit.
  const handleChangeField = (field: keyof StaffFormValues, value: string) => {
    setFormValues((current) => ({ ...current, [field]: value }));
  };

  // Submit form: chặn submit mặc định, hiện lỗi inline khi Zod/username không hợp lệ; hợp lệ thì dispatch ADD/UPDATE và báo thành công.
  ...
  // Click badge trạng thái: dispatch toggle trên mock state và báo active/locked mới; không gọi API phân quyền.
  ...
  // Xác nhận xóa: guard khi không còn bản ghi chờ, dispatch DELETE cục bộ rồi đóng hộp thoại và báo thành công.
