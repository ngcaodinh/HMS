/**
 * @file PharmacyWorkspace.tsx
 * @description Component điều phối chính cho Phân hệ Dược sĩ và Nhà thuốc HMS-VN.
 * Tích hợp điều hướng năm màn hình chức năng, dữ liệu kho/đơn thuốc, mutation cấp phát/từ chối,
 * modal xác nhận và phản hồi Toast trong cùng một workspace.
 */
...
import { MedicalErrorBadge, MedicalSuccessBadge } from '@/shared/components/AppToast';
 ...
 * `type` quyết định biểu tượng và bảng màu; thông báo tự ẩn sau thời gian hiển thị mặc định.
 */
...
/** Định dạng ngày ISO thành ngày theo locale `vi-VN`, với fallback khi chưa có dữ liệu. */
function formatDateVN(value?: string | null): string {
  if (!value) return 'Chưa ghi nhận';
  return new Date(value).toLocaleDateString('vi-VN');
}

/** Định dạng thời điểm ISO thành ngày giờ theo locale `vi-VN`, với fallback khi chưa có dữ liệu. */
function formatDateTimeVN(value?: string | null): string {
  if (!value) return 'Chưa ghi nhận';
  return new Date(value).toLocaleString('vi-VN');
}

/** Tính tuổi tròn theo ngày sinh và ngày hiện tại; trả về `0` khi thiếu ngày sinh. */
...
/** Chuẩn hóa lỗi API thành thông báo an toàn để hiển thị trên Toast. */
function getApiErrorMessage(error: unknown): string {
  return error instanceof ApiError ? error.message : 'Yêu cầu không thành công. Vui lòng thử lại.';
}

/** Lấy lỗi validation của một field từ envelope lỗi API nếu backend có trả về. */
...
/**
 * Component Không gian làm việc chính của Dược sĩ.
 ...
 * @remarks
 * Dữ liệu đơn thuốc, tồn kho, kho và biến động kho đến từ các query tương ứng; dữ liệu mock chỉ
 * được dùng làm fallback hiển thị khi API không có bản ghi. Các mutation cấp phát, từ chối và
 * kết xuất XML cập nhật state cục bộ sau thành công, hiển thị lỗi/Toast và refetch khi backend
 * báo xung đột optimistic lock hoặc đơn đã được xử lý ở nơi khác. Workspace có loading, empty,
 * error và success state thông qua các màn hình con; UI gate không thay thế authorization ở backend.
 */
export function PharmacyWorkspace() {
  ...
  // Danh sách đơn hiển thị; effect đồng bộ API và dùng fallback mock khi query không có bản ghi.
  ...
  // Trạng thái draft phiếu nhập kho.
  ...
  // Query dữ liệu tồn kho, tổng hợp tồn, kho và nhật ký biến động từ backend.
  ...
  /**
   * Đồng bộ hàng chờ server vào model hiển thị của workspace khi query hoặc trạng thái tải thay đổi.
   * Effect map snapshot backend sang kiểu UI, giữ đơn đang chọn nếu còn tồn tại và dùng danh sách
   * fallback khi query đã kết thúc nhưng không có dữ liệu.
   */
  useEffect(() => {
  ...
  /**
   * Nạp nội dung XML khi chuyển sang màn tra cứu hoặc đổi đơn đang chọn.
   * Request gọi API bên ngoài state React; cờ hủy ngăn response cũ ghi đè đơn mới, còn cleanup
   * đánh dấu request không còn hợp lệ khi dependency thay đổi hoặc component unmount.
   */
  useEffect(() => {
   ...
   * @remarks Thêm Toast vào state cục bộ và đăng ký timer tự xóa sau 4 giây.
   */
  ...
  /**
   * Xử lý sự kiện xác nhận phát thuốc cho đơn đang chọn.
   * Guard kiểm tra trạng thái pending, hóa đơn đã thanh toán và đủ tồn theo phân bổ FEFO trước
   * khi gửi mutation; backend vẫn là nguồn quyết định cuối cùng và nhận `expectedVersion` để
   * bảo vệ optimistic lock. Thành công cập nhật trạng thái cục bộ, đóng modal và báo Toast;
   * lỗi nghiệp vụ hiển thị cảnh báo, còn lỗi xung đột version/đã phát sẽ refetch hàng chờ.
   */
  ...
  /**
   * Xử lý sự kiện từ chối đơn thuốc và gửi phản hồi về cho bác sĩ kê đơn.
   * Guard chỉ cho phép đơn pending; mutation gửi lý do cùng `expectedVersion`, hiển thị lỗi field
   * hoặc lỗi hệ thống, đóng modal khi thành công và refetch khi optimistic lock bị xung đột.
   *
  ...
  /**
   * Xử lý sự kiện xác nhận nhập dữ liệu XML phiếu nhập kho.
   * Callback này cập nhật draft phiếu nhập ở workspace, hiển thị Toast thành công và đóng modal;
   * việc xác thực hoặc lưu bền vững do luồng nghiệp vụ bên ngoài component này đảm nhiệm.
   */
  ...
  /**
   * Cập nhật khoảng ngày báo cáo và ngăn query khi khoảng ngày không hợp lệ.
   * Giá trị ngày dùng định dạng `YYYY-MM-DD` từ input date; `dateError` được truyền xuống màn hình
   * báo cáo và đồng thời làm query bỏ qua khoảng ngày sai.
   */
   ...
   * Ủy quyền side effect đăng xuất cho hook phiên làm việc; modal chỉ đóng sau khi flow cha quyết định.
   */
  ...
  /**
   * Xử lý sự kiện in nhãn hướng dẫn sử dụng thuốc.
   * Tạo cửa sổ in mới từ dữ liệu đơn hiện tại, escape nội dung động trước khi đưa vào HTML và báo
   * lỗi nếu trình duyệt chặn popup; Toast chỉ xác nhận đã mở hộp thoại in.
   */
  ...
  /**
   * Xử lý sự kiện kết xuất XML cho một đơn thuốc.
   * Mutation gửi ID backend và `expectedVersion`; thành công cập nhật version/thời điểm kết xuất
   * từ response. Khi mutation lỗi, giữ fallback hiện tại của UI bằng cách đánh dấu thời điểm cục
   * bộ và cho phép luồng tải XML tiếp tục dùng nội dung được tạo từ snapshot đơn.
   */
  ...
  /**
   * Tải XML đã kết xuất theo ID backend, fallback sang Blob tạo từ snapshot nếu tải API thất bại.
   * Side effect gồm tạo anchor tải tệp, thu hồi object URL và luôn trả state loading về `false`.
   */
  ...
  // Dữ liệu mock chỉ là fallback hiển thị cho kho, đơn thuốc và báo cáo khi API không có bản ghi.
  const warehouses =
  ...
  return (
    <div className={styles.shell}>
      {/* Thanh Sidebar điều hướng của workspace Dược. */}
      <PharmacySidebar
      ...
      />
        ...
        {/* Khu vực nội dung chính của workspace. */}
      <div className={styles.mainWrap}>
        {/* Header chứa các thao tác chung của phân hệ. */}
        <PharmacyTopHeader
          onShowNotifications={() =>
        ...
        {/* Nội dung thay đổi theo màn hình đang chọn. */}
        <main className={styles.contentArea}>
          ...
          {/* Màn hình cấp phát thuốc theo đơn. */}
          ...
          {/* Màn hình quản lý kho thuốc và lô FEFO. */}
          ...
          {/* Màn hình tra cứu đơn thuốc và XML quốc gia. */}
          ...
          {/* Màn hình báo cáo biến động kho. */}
      ...
      {/* Tập hợp các hộp thoại tương tác của phân hệ. */}
      <PharmacyModals
      ...
      {/* Khung Toast cục bộ; mỗi Toast tự biến mất theo timer trong showToast hoặc nút đóng. */}
