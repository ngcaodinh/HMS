...
/**
 * Props của worklist cấp phát thuốc.
 * `statusFilter` là bộ lọc hiển thị theo `dispensedAt`; nó không thay thế status domain hoặc paid gate
 * do backend kiểm tra. Các callback chỉ cập nhật query/selection ở parent.
 */
...
/**
 * Hiển thị danh sách đơn trong worklist pharmacy.
 *
 * @param isLoading Hiển thị trạng thái đang tải khi query chưa có dữ liệu ổn định.
 * @param keyword Từ khóa hiện tại và callback thay đổi từ khóa.
 * @param list Danh sách đơn đã được API/query parent lọc và phân trang.
 * @param onChangeKeyword Callback cập nhật keyword ở parent; không tự gọi API.
 * @param onChangeStatusFilter Callback đổi tab `pending`/`dispensed`.
 * @param onSelect Callback chọn một đơn để parent tải/hiển thị chi tiết.
 * @param selectedId ID đang được chọn, có thể null khi chưa chọn.
 * @param statusFilter Tab trạng thái hiện tại.
 * @returns Bảng worklist với loading, empty và success/list states.
 * @remarks Lỗi query không được xử lý trong component vì không có prop error; parent phải render lỗi.
 * Allergy badge chỉ đếm override đã có trong response. UI visibility không thay thế permission backend.
 */
export function DispenseList({
  ...
  // Chỉ hiển thị số override đã được server ghi nhận; client không tự đánh giá hay ghi đè dị ứng.
