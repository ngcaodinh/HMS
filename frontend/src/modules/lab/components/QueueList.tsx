...
/** Tab lọc cục bộ: tất cả trạng thái, chỉ `ordered`, hoặc mọi phiếu có cờ cấp cứu. */
...
/** Tập cờ quyết định nút nhập kết quả/in theo trạng thái server của phiếu. */
...
/** Ánh xạ trạng thái server sang nhãn hiển thị mà không thay đổi giá trị trạng thái gốc. */
...
/**
 * Hiển thị worklist xét nghiệm, bộ lọc cục bộ và các thao tác theo trạng thái phiếu.
 *
 * @param props.filterTab Tab `all`/`ordered`/`urgent` đang chọn.
 * @param props.isLoading Trạng thái tải worklist; khi true hiển thị hàng loading.
 * @param props.keyword Từ khóa lọc theo tên, mã bệnh nhân hoặc mã phiếu.
 * @param props.list Danh sách từ query server, gồm status mẫu/kết quả và cờ cấp cứu.
 * @param props.onChangeFilterTab Callback cập nhật tab lọc ở workspace cha.
 * @param props.onChangeKeyword Callback cập nhật từ khóa ở workspace cha.
 * @param props.onPrint Callback để cha tạo bản in; component không tự gọi API in.
 * @param props.onSelect Callback phát mã phiếu sang luồng nhập kết quả.
 * @returns UI loading, empty hoặc bảng worklist thành công cùng số liệu tóm tắt.
 * @remarks Component chỉ quyết định visibility của nút theo status; backend vẫn quyết định quyền,
 * transition và validation. Dữ liệu list do workspace/API cung cấp, filter/search là local state.
 */
            ...
            {!isLoading &&
              filtered.map((item, index) => (
                <tr className={cn(item.isUrgent && 'bg-[#fff5f4]')} key={item.labTestId}>
                  ...
                  {/* Schema chưa có barcode thật; mã này chỉ là số thứ tự của danh sách đã lọc, không phải mã backend. */}
                  <td className={styles.td}>
