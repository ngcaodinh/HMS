 ...
 * @access doctor
 */
export async function createPrescriptionDraftController(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    const input = createPrescriptionDraftSchema.parse(req.body);
    const result = await createPrescriptionDraft(recordId, principal.userId, input);
 ...
 * @access doctor
 */
export async function getLatestPrescriptionController(
  req: Request,
  res: Response,
  next: NextFunction,
) {
...
export async function signPrescriptionController(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const input = signPrescriptionSchema.parse(req.body);
    const result = await signPrescription(prescriptionId, principal.userId, input);
    sendSuccess(res, result);
  } catch (error) {
    next(error);
  }
 ...
 */
export async function cancelPrescriptionController(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    const principal = requirePrincipal(req);
    const { prescriptionId } = req.params as { prescriptionId: string };
    const input = cancelPrescriptionSchema.parse(req.body);
    const result = await cancelPrescription(prescriptionId, principal, input);
    sendSuccess(res, result);
  } catch (error) {
    next(error);
  }
 ...
 * @access doctor, pharmacist, admin
 */
export async function exportPrescriptionXmlController(
  req: Request,
  res: Response,
  next: NextFunction,
) {
 ...
 * @desc Tải file nhị phân — trả XML thô, không bọc envelope JSON.
 * @access doctor, pharmacist, admin
 */
export async function downloadPrescriptionXmlController(
  req: Request,
  res: Response,
  next: NextFunction,
) {
 ...
 */
export async function listDispensablePrescriptionsController(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    const result = await listDispensablePrescriptions({
      keyword,
      dispensed,
      page,
      pageSize,
 ...
 * @access pharmacist, admin
 */
export async function dispensePrescriptionController(
  req: Request,
  res: Response,
  next: NextFunction,
) {
